...
function createCartela(){
   // Descrição: Gerar randomicamente uma nova cartela
    ...
    while(qcolunas < 2){
        qcolunas=Math.floor((Math.random() * 10) + 1);
    }
...
function updateCartela(linha, coluna, num){
    ...
    if(linha < qlinhas && linha >= 0 && coluna < qcolunas && coluna >= 0){
    //indice valido
        ...
        if(usados[linha][coluna] == 0 && tab[linha][coluna] == num){
            ...
            console.log("Posição", linha, coluna, "foi atualizada.");
            return true;
        }
